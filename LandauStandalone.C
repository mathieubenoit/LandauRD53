...
double gsl_ran_landau(double r)
{
  static double F[983] =
    {
      ...
      0.051376, 0.056997, 0.062620, 0.068247, 0.073877,
      ...
      0.337654, 0.343515, 0.349382, 0.355255, 0.361135,
      ...
      0.639689, 0.645945, 0.652210, 0.658484, 0.664768,
      ...
      0.965614, 0.972439, 0.979278, 0.986130, 0.992996,
      ...
      1.325203, 1.332819, 1.340454, 1.348108, 1.355780,
      ...
      1.731303, 1.740011, 1.748746, 1.757506, 1.766293,
      ...
      2.202018, 2.212257, 2.222533, 2.232845, 2.243195,
      ...
      2.764563, 2.777012, 2.789514, 2.802070, 2.814681,
      ...
      3.462711, 3.478500, 3.494372, 3.510328, 3.526370,
      ...
      4.480154, 4.502060, 4.524114, 4.546319, 4.568676,
      ...
      5.642734, 5.673552, 5.704634, 5.735986, 5.767610,
      ...
      7.605785, 7.656608, 7.708035, 7.760077, 7.812747,
...
double Formula(double Lambda,int Nbit_final,double Ep ,double mass , double x,double Threshold ,double Overflow ){
...
int GetABitcode(){
	...
	//R is an uniform number on ]0,1[
	double R = (double)rand()/RAND_MAX;
	while((R==0) | (R==1)){
		R = rand()/RAND_MAX;
		}
	int bc = Formula(gsl_ran_landau(R),Nbit_final,Ep,mass,x,Threshold,Overflow);
...
void GeneratePDF(int Nbit_final,double Ep ,double mass , double x,double Threshold ,double Overflow,double *pdf,int NMax =1000) { 
	...
	double dx = (xmax-xmin)/(niter-1);
	double xs=xmin;
	
	for(int i=0;i<niter;i++){
	
		int index = floor(Formula(xs,Nbit_final,Ep,mass,x,Threshold,Overflow));
		
		if(index>=0 && index<NMax){		
			pdf[index] += gsl_ran_landau_pdf(xs)*dx;
		}
...
void LandauStandalone(){
	...
	double Threshold = 5;
	double Overflow = 50;	
	//maximum bin of the PDF
	int NMax = 1000;
	

	//Parameter Vebose 
	
	cout << "[PDFGenerator] Generating PDF for the following parameters : " << endl;
	cout << "[PDFGenerator] " << "Final counter depth : " << Nbit_final << " bits" << endl;	
	cout << "[PDFGenerator] " << "Particle Energy : " << Ep/1e6 << " GeV" << endl;	
	cout << "[PDFGenerator] " << "Particle mass : " <<mass << " keV" << endl;	
	cout << "[PDFGenerator] " << "Sensor thickness : " << x*1e4  << " um" << endl;	
	cout << "[PDFGenerator] " << "Threshold : " << Threshold << " keV" << endl;	
	cout << "[PDFGenerator] " << "Overflow : " << Overflow << " keV" << endl;	
	cout << "[PDFGenerator] " << "Generating up to bin " <<NMax  << endl;	

	
	
		
	char buffer [256];
	sprintf(buffer,"Landau_PDF_%ibits_E=%08.1fkeV_m=%.1fkeV_x=%fcm_T=%.1fkeV_O=%.1fkeV_NMax=%i.txt",Nbit_final,Ep,mass,x,Threshold,Overflow,NMax);
	
		
	cout << "Generating PDF " << buffer << endl;
	GeneratePDF(Nbit_final,Ep,mass,x,Threshold,Overflow,pdf,NMax);
